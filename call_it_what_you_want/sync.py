"""Keep the team registry covering the games a consumer actually replays.

    ciwyw sync ncaafb                # pull, reconcile, record
    ciwyw sync ncaafb --check        # report, record nothing
    ciwyw sync ncaafb --yes          # don't stop to ask

Part of the `sync` extra: it talks to ESPN and reads seasons out of s3,
neither of which the read side of this package depends on.

`record` appends discoveries to the local staging file and they take effect
for every caller immediately. This module only decides *what* to record. One
...
1. *Pull.* ESPN's scoreboard carries a team id next to every name, and
   ...
   seasons from, so what lands here lines up with the stored team strings
...
What gets recorded lands in the local staging file (`ciwyw where`), and

    ciwyw show --output call_it_what_you_want/data/ncaa.csv

is how it becomes a diff to review. One file covers all of college sports,
since ESPN numbers teams per organization and every college sport shares
those ids.
"""
...
from endgame.types import NcaaFbGroup, Season, SeasonType
...
from .data import COLUMNS, NewRecordWarning, default_teams, record
from .local import append_local, local_path
from .namer import TeamNamer, namespace_for, registry_league
from .registry import Teams, UnknownTeamError, normalize
from .types import ESPN, AmbiguousNameError, NoNamesError, Team, TeamName

# How endgame_aws lays seasons out in the bucket. Matched here rather than
# imported because it isn't part of that package's API -- it's the shape of
# the keys its `main.py` writes.
_SEASON_KEY_RE = re.compile(r"^seasons/(\d+)/([^/]+)\.pkl$")


async def read_all_seasons(league: str, bucket: str) -> AsyncIterator[Season]:
    """Every stored season for a league, in whatever order the bucket lists."""
...
class Decisions:
    ...
    Kept next to the staging file: it's the same kind of thing -- local
    state about local discoveries -- and it should not follow the repo to
    another machine.
    """
...
def _review_names(
    ...
    `registry_league` is the registry's name for the sport, not endgame's.
    """
...
def _sync(
    ...
    if namespace is None or reg_league is None:
        raise ValueError(
            f"League {league!r} has no ESPN id namespace. "
...
def main(
    ...
    """
    Pull a league's names from ESPN, reconcile them against the seasons,
    and stage what's new.

    `league` is endgame's name for it -- `ncaafb`, `mens`, `womens` --
    since that's what its seasons are stored under.
