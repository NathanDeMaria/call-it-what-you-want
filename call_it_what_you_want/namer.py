"""Applying the registry to a feed's games: renaming teams to one name each.

The registry answers who a team is. This is the adapter that puts that
answer to work on a stream of games, and decides what to do about the names
it can't place.
...
name starts over at the middle of the scale. Both halves are wrong, and the
abandoned one is worse, because a frozen rating never regresses and still
sorts into the rankings.
...
Names are canonicalized here rather than in the stored seasons, so a bad
correction is undone by editing one file and re-running.
"""

from collections import Counter
from typing import TYPE_CHECKING, Self

from .data import NCAA, default_teams
from .registry import AmbiguousTeamError, Teams, UnknownTeamError
from .types import NCAAFB, NCAAMBB, NCAAWBB, AmbiguousNameError, NoNamesError

if TYPE_CHECKING:
    # Only for the signature of `apply`. Importing it for real would put
    # endgame in the base install, and the whole point of the read side is
    # that translating a name costs no dependencies.
    from endgame.types import Game

# The league names `endgame` uses -- and so the ones its seasons are stored
# under -- mapped to what this package records names in. They agree on
# `ncaafb` and nowhere else: endgame's `mens` is `ncaambb` here. Anything
# writing an observation has to translate, since a name filed under `mens`
# is invisible to every lookup.
#
# A league that isn't here gets no renaming at all.
...
def registry_league(league: str) -> str | None:
    """What this package calls one of endgame's leagues, if it knows it."""
...
class TeamNamer:
    ...
    def for_league(cls, league: str) -> Self:
        ...
        A league with no namespace -- `nfl`, or anything picked up later --
        is not an error: it means every name passes through, which is what a
        league with stable names wants anyway.

        `default_teams` layers whatever a sync run recorded locally over
        what shipped with the package, so a rename found today is in effect
        today rather than waiting on a release. `ciwyw where` is that
        staging file, and `ciwyw show --output` is how those rows get
        upstreamed.
        """
