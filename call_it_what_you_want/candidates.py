"""Find the names the registry doesn't cover yet, and where they belong.

The rest of this package answers *who a team is*; this answers *which names
it is still missing*, by looking at the seasons a consumer actually replays.
The output is rows in the registry's own format -- `espn_id,name,year,source`
-- so a finding here becomes a staged row, with no translation in between.

Part of the `sync` extra: this reads `endgame` seasons, which the read side
of the package deliberately doesn't depend on.
...
def unresolved_names(
    ...
    team in every game the seasons contain has an identity.
    """
    unresolved = {
